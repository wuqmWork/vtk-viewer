        ...
        std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                if available.is_empty() {
                    for d in 1..=config.preload_layers {
                        if let Some(prev) = current.checked_sub(d) { targets.push(prev); }
                        let next = current.saturating_add(d);
                        if next < total_layers { targets.push(next); }
                    }
                } else {
                    ...
                    for d in 1..=config.preload_layers as usize {
                        if pivot >= d { targets.push(available[pivot - d]); }
                        let next_index = if current_index.is_some() { pivot + d } else { pivot + d - 1 };
                        if let Some(next) = available.get(next_index) { targets.push(*next); }
                    }
                }
