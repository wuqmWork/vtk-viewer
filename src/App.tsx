...
import { getGalvoLayout, scanDataset, selectDatasetFolder, type LayerRequestMode } from './services/tauri'
  ...
  useEffect(() => {
    ...
    getGalvoLayout()
      .then(layout => {
        ...
        setGalvos(DEFAULT_GALVOS.map(base => {
          const source = byId.get(base.id)
          return { ...base, folder: source?.folder, transform: source?.transform }
        }))
      })
      .catch(error => console.error('加载振镜布局失败', error))
    return () => { active = false }
  }, [])
