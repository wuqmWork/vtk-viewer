...
export class VtkScene {
  ...
  private gridActors: any[] = []
  private globalAxisActors: any[] = []
  ...
  mount(container: HTMLElement) {
    ...
    this.gridActors = [grid.minor, grid.major]
    this.gridActors.forEach(actor => this.renderer.addActor(actor))
    this.globalAxisActors = createGlobalAxes()
  ...
  setGridVisible(visible: boolean) {
    ...
    this.gridActors.forEach(actor => actor.setVisibility(visible))
    this.renderWindow.render()
  }
  ...
  requestLayer(layer: number, mode: LayerRequestMode = 'exact') {
    // totalLayers = 0 时夹到 0，其余情况夹到 [0, totalLayers - 1]。
    const clamped = Math.max(0, Math.min(this.totalLayers - 1, layer))
  ...
  private rebuildCoordinateActors() {
    ;[...this.globalAxisActors, ...this.localAxisActors].forEach(actor => this.renderer.removeActor(actor))
    this.globalAxisActors = []
    this.localAxisActors = []

    if (this.coordMode === 'global') {
      this.globalAxisActors = createGlobalAxes()
      this.globalAxisActors.forEach(actor => this.renderer.addActor(actor))
    } else if (this.coordMode === 'local') {
      for (const galvo of this.galvos) {
        if (!galvo.transform) continue
        const axes = createLocalAxes(galvo.transform)
        axes.forEach(actor => actor.setVisibility(galvo.visible))
        this.localAxisActors.push(...axes)
      }
      this.localAxisActors.forEach(actor => this.renderer.addActor(actor))
    }
    this.renderWindow.render()
  }
}
